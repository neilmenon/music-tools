...
export class LastfmService {
  ...
  spotifyAlbumModel.data.forEach(spotifyAlbum => {
    ...
    if (mostLikelySoundtrackCompilation) {
      ...
      const albumDistinctTracksListenQualifier: number = Math.ceil(spotifyAlbum.custom.adjustedFullPlayTracks * 0.75)
      const freebeeMax: number = Math.ceil(spotifyAlbum.custom.adjustedFullPlayTracks / 10)
